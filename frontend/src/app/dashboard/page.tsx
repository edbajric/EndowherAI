...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      if (logs && logs.length > 0) {
        ...
        const avgPain =
          Math.round((logs.reduce((sum, l) => sum + (l.pain_level || 0), 0) / totalLogs) * 10) / 10;
        const avgFatigue =
          Math.round((logs.reduce((sum, l) => sum + (l.fatigue_level || 0), 0) / totalLogs) * 10) /
          10;
  ...
  return (
    <PageShell title="Dashboard" subtitle="Your complete tracking overview.">
      {/* Stats Grid */}
                ...
                <tbody>
                  {recentLogs.map(log => (
                    <tr key={log.id} className="border-b border-ink/5 text-ink">
                      ...
                      <td className="py-2 text-xs max-w-37.5 truncate">{log.notes || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
