...
export default function HomePage() {
  ...
  function handleAssessmentResult(result: AssessmentResult) {
    ...
    setChatLime(getVisibleLIMEFactors(result.explanation));
  }
  ...
  useEffect(() => {
    async function load() {
      ...
      if (allLogs && allLogs.length > 0) {
        ...
        const avgPain =
          Math.round((allLogs.reduce((s, l) => s + (l.pain_level || 0), 0) / totalLogs) * 10) / 10;
        const avgFatigue =
          Math.round((allLogs.reduce((s, l) => s + (l.fatigue_level || 0), 0) / totalLogs) * 10) /
          10;
    ...
    <PageShell title="Welcome back" subtitle="Your health snapshot and history.">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-success to-[#a8c0a0] px-5 py-4 text-center text-white shadow-sm">
          <p className="text-3xl font-bold">
            {stats.streak} <span className="text-sm opacity-70">d</span>
          </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-inkMuted">
                {todayLog ? todayLog.log_date : 'No entry yet'}
              </p>
            </div>
            {todayLog && (
        ...
        <div className="rounded-3xl bg-bgSoft ring-1 ring-success/15 px-6 py-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-inkMuted">
                {weeklyLog ? `Week of ${weeklyLog.week_start}` : 'No check-in yet'}
              </p>
            </div>
            {weeklyLog && (
              <span className="rounded-full bg-success/20 px-3 py-1 text-xs font-medium text-success">
        ...
        <Card title="Recent symptom entries" description="Your last 10 diary logs.">
          {recentLogs.length === 0 ? (
            <div className="mt-2">
              ...
              <Button href="/diary" variant="secondary">
                Log symptoms
              </Button>
            </div>
          ) : (
        ...
        <Card title="Recent remedies" description="Your last 5 remedy entries.">
          {recentRemedies.length === 0 ? (
            <div className="mt-2">
              ...
              <Button href="/remedies" variant="secondary">
                Log a remedy
              </Button>
            </div>
          ) : (
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-3">
        ...
        <Button fullWidth variant="secondary" href="/weekly">
          Weekly check-in
        </Button>
