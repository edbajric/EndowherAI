        ...
        <div className="max-w-2xl space-y-5">
          <div className="rounded-3xl bg-green-50 border border-green-200 px-5 py-4">
            <p className="text-sm font-semibold text-green-700">
              Weekly check-in saved successfully.
            </p>
          </div>
            ...
            <div className="rounded-3xl ring-1 ring-primary/15 px-6 py-5 bg-bg shadow-sm space-y-4">
              ...
              <div className={`rounded-2xl border px-4 py-3 ${style.bg} ${style.border}`}>
                ...
                <p className="mt-2 text-xs text-inkMuted">
                  Probability your symptom pattern matches a diagnosed / suspected PCOS or
                  Endometriosis profile in our research cohort.
                </p>
              </div>

              {getVisibleLIMEFactors(aiResult.explanation).length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-inkMuted">
                    Key factors
                  </p>
                  {getVisibleLIMEFactors(aiResult.explanation).map((f: LIMEFactor, i: number) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 rounded-2xl bg-bgMuted/40 px-3 py-2.5"
                    >
                      <span
                        className={`mt-0.5 shrink-0 h-5 w-5 rounded-full flex items-center justify-center text-xs font-bold text-white ${f.direction === 'increases_risk' ? 'bg-red-400' : 'bg-green-400'}`}
                      >
              ...
              <p className="text-xs text-inkMuted italic">
                ⚕️ Not a diagnosis. For educational insights based on research patterns only.
      ...
      <div className="max-w-2xl">
        ...
        {step === 1 && (
          <Card>
            <StepHeader
              step={1}
              title="Pain Severity"
              subtitle="Rate each type of pain 0–10 (0 = none, 10 = worst possible)"
            />
            <div className="space-y-5">
              <Slider010
                label="Period pain / cramps (dysmenorrhea)"
                value={periodPain}
                onChange={setPeriodPain}
              />
              <Slider010
                label="Pelvic pain outside your period"
                value={pelvicPain}
                onChange={setPelvicPain}
              />
              <Slider010
                label="Abdominal pain or pressure"
                value={abdominalPain}
                onChange={setAbdominalPain}
              />
              <Slider010
                label="Lower back pain related to your cycle"
                value={lowerBackPain}
                onChange={setLowerBackPain}
              />
              <Slider010
                label="Hip or leg pain related to your cycle"
                value={hipLegPain}
                onChange={setHipLegPain}
              />
              <Slider010
                label="Sharp / stabbing pelvic pain"
                value={sharpPain}
                onChange={setSharpPain}
              />
              <Slider010
                label="Overall chronic pain (all combined, last 3 months)"
                value={overallPain}
        ...
        {step === 2 && (
          <Card>
            <StepHeader
              step={2}
              title="Bleeding, Cycle & Bloating"
              subtitle="Patterns over the past week / cycle"
            />
            <div className="space-y-5">
              <PillGroup
              ...
              />
              <PillGroup
                label="Bloating / endo belly frequency"
                value={bloating}
              ...
              />
              <PillGroup
                label="Dizziness / fainting around your period"
                value={dizziness}
        ...
        {step === 3 && (
          <Card>
            <StepHeader
              step={3}
              title="Body & Mind"
              subtitle="Systemic symptoms and quality of life"
            />
            <div className="space-y-5">
              <PillGroup
                label="Tiredness / exhaustion not improved by sleep"
                value={fatigue}
                onChange={setFatigue}
                options={['Never', 'Rarely', '1–3 d/mo', '4–10 d/mo', '>10 d/mo', 'Other']}
              />
              <PillGroup
                label="Headaches / migraines related to cycle"
                value={headaches}
                onChange={setHeadaches}
                options={['Never', 'Sometimes', 'Often']}
              />
              <Likert
                label="Mood swings, anxiety or depression (cycle-related)"
                value={moodAnxiety}
                ...
                options={[
                  ...
                  { label: 'Some cycles', value: 2 },
                  { label: 'Most cycles', value: 3 },
                  { label: 'Every cycle', value: 4 },
                ]}
              />
              <Slider010
                label="Quality of life impact (0 = not affected · 10 = severely)"
                value={qualityOfLife}
              ...
              <SensitiveSection
                ...
                hint="Optional — commonly reported with endometriosis"
              >
                <div className="space-y-4 pt-2">
                  ...
                  />
                  <PillGroup
                    label="Pain / burning when urinating"
                    value={urinaryPain}
                    onChange={setUrinaryPain}
                    options={['Never', 'Sometimes', 'Often']}
                  />
                </div>
              </SensitiveSection>
        ...
        {step === 4 && (
          <Card>
            <StepHeader
              ...
              subtitle="Improves AI accuracy — all fields optional"
            />
            <div className="space-y-5">
              ...
              <Slider010
                label="Painful ovulation (mid-cycle pain)"
              ...
              <Slider010
                label="Anti-inflammatory diet effectiveness (0 = not tried)"
                value={antiInflDiet}
                onChange={setAntiInflDiet}
                optional
              />
              <Slider010
                label="Pelvic floor / physiotherapy effectiveness"
                value={pelvicFloor}
                onChange={setPelvicFloor}
                optional
              />
              <div>
                <label className="text-sm font-medium text-inkStrong">
                  Notes <span className="font-normal text-inkMuted">(optional)</span>
                </label>
                <textarea
                  rows={3}
                  placeholder="Patterns, triggers, or context for this week…"
                ...
                <p className="mt-1 text-xs text-inkMuted">
                  Avoid including names or other identifying details.
                </p>
              </div>
            </div>
          </Card>
          ...
          <div className="mt-10">
            ...
            <div className="space-y-3">
              {logs.map(l => (
                <div
                  key={l.id}
                  className="rounded-2xl bg-bgMuted/40 px-4 py-3 flex items-center justify-between"
                >
                  <div>
                  ...
                  {l.notes && <p className="text-xs text-inkMuted max-w-35 truncate">{l.notes}</p>}
                </div>
              ))}
            </div>
          </div>
